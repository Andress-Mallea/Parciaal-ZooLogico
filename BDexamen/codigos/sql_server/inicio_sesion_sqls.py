...
def inicio_sesion(main_menu):
    ...
    def hola():
        if server_entry.get() == '' or user_entry.get() == '' or password_entry.get() == '':
            messagebox.showwarning("ERROR", "Llena los datos antes de ingresar")
        else:
            servidor = server_entry.get()
    ...
    nombre_u.grid(row="2", column="0", padx=10, pady=10)
    ...
    nombre_p.grid(row="3", column="0", padx=10, pady=10)
    ...
    mostrar_ocultar_boton.grid(row="3", column="2", padx=5, pady=5)
